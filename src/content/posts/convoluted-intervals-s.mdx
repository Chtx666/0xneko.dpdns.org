...
稍微動點腦筋，不難看出差分以統計答案，優化掉$O(m)$的複雜度。
...
經過查題解，筆者發現，$n\in[0,200000]$，而$m\in[1,5000]$，真相只有一個！！！

我們可以化$n$為$m$，即對區間的$a$、$b$的值進行計數處理（不考慮區間對應關係及順序），存進$bka$、$bkb$兩個桶（用數組實現）；
然後遍歷其（$O(m^2)$），對差分數組進行處理（$O(1)$），最後對$ans$做前綴和即可。
...
1.由於方案數為$bka$的二次方數量級，要開$long\ long$；
...
ans[i + j + 1] -= bkb[i] * bkb[j];
```
爲什麽在$i+j$処要增加bka的情況，而緊隨其後就要減少$bkb$呢？

這兩者明明並不存在任何區間上或值上的關聯啊。

原來$i+j$代表區間開端的值，其實這段代碼就相當於
